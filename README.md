...
- 三个自定义永续合约交易对，分别设置 0～8 位小数。
- Binance、OKX、Bybit 三个公开行情源，支持自动故障切换和固定数据源。
- 优先使用 WebSocket 实时推送，断线自动重连；推送不可用时使用 HTTP 查询兜底。
- 可配置 SOCKS5 / HTTP 代理，默认启用 `socks5://127.0.0.1:7897`，用于行情和图标请求。
...
- 深色科技风中文设置页（默认 400×560，正文 11 像素），深蓝分层卡片与青蓝强调色，数值直接输入，外观预览按需展开。
...
本项目显示交易所公开永续合约接口的最新成交价，不需要 API Key，不提供交易、资金、杠杆、走势图或系统托盘功能。
...
- **单击悬浮窗**：立即检查行情并在推送不可用时请求兜底报价；推送正常时持续实时更新，请求进行中不会重复发送。
...
- **交易对**：统一填写 `BTCUSDT`、`ETHUSDT`、`SOLUSDT` 等永续合约代码，输入自动去除首尾空格并转大写；OKX 会自动转换为 `BTC-USDT-SWAP` 等格式。代码格式合法不代表各交易所都支持该合约，不支持时自动模式会继续尝试下一源。
- **数值设置**：字号、背景不透明度、行情兜底间隔、轮播间隔和小数位均使用数值输入框，支持直接键入，单位显示在框内。行情兜底间隔仅控制推送异常时的 HTTP 查询，不限制正常推送速度。迷你模式字号固定为 12 像素，关闭迷你模式后可编辑完整模式字号。
...
- **报价单位**：显示永续合约的计价币种，例如 USDT 或 USDC，不将其统一当作美元。
...
设置页的“网络代理”可修改协议、主机地址和端口（1～65535），保存后立即生效；取消勾选“启用代理”并保存可改为直连。地址仅填写 IP 或主机名，不包含协议和端口。新配置及缺少代理字段的旧配置默认启用 SOCKS5 `127.0.0.1:7897`；本机代理未运行时，请启动代理或关闭该开关。

代理配置字段为 `proxy_enabled`、`proxy_type`（`socks5` / `http`）、`proxy_host`、`proxy_port`，与其他设置一起保存。修改代理会取消旧连接上的请求，重新获取行情与图标。
...
行情优先通过以下公开 WebSocket 接口订阅，各币种独立连接与故障切换，无需 API Key：

| 数据源 | WebSocket 地址 | 订阅 |
| --- | --- | --- |
| Binance | `wss://fstream.binance.com/market/ws/btcusdt@ticker` | 单币种 24 小时行情中的最新成交价 `c` |
| OKX | `wss://ws.okx.com:8443/ws/v5/public` | `tickers`，`instId=BTC-USDT-SWAP` |
| Bybit | `wss://stream.bybit.com/v5/public/linear` | `tickers.BTCUSDT`，支持增量消息 |

自动模式从 Binance 开始，断线、订阅失败或连续 30 秒没有有效行情时，按 Binance → OKX → Bybit 循环尝试；健康连接保持当前来源，避免来回切换。固定源模式只重连所选交易所。重试间隔从 1 秒递增到最多 30 秒，收到有效行情后重置；每 15 秒发送心跳。Bybit 增量中省略成交价表示价格未变化，沿用本次连接最近一次成交价。

每个币种将 250 毫秒内的推送合并为最新一条，减少界面重绘。切换设置、代理或退出时关闭旧连接并丢弃待显示报价；较早发出的 HTTP 响应不会覆盖健康推送。WebSocket 与 HTTP 使用相同的代理设置。

启动连接期间、连接断开或推送过期时，通过以下 HTTP 接口兜底；健康推送期间暂停周期性 HTTP 行情请求：
...
HTTP 兜底在自动模式下，对每个交易对分别按 **Binance → OKX → Bybit** 顺序请求，首个有效报价即结束本轮。超时、网络或交易所业务错误、空数据、交易对不匹配都会触发下一源；所有源失败才显示错误。下一轮 HTTP 查询仍从 Binance 开始。在途请求整条重试链去重，避免刷新周期重叠。固定源模式仅访问所选交易所。

三个源均使用**永续合约**，Binance 使用 USDⓈ-M Futures，Bybit 使用 `linear`，OKX 使用 `SWAP`。OKX 响应会检查 `SWAP` 类型及交易对代码；不能识别计价币种时跳过 OKX，不猜测交易对。各交易所对不同计价币种的支持可能不同，组件不取均价、不同时显示三份报价。
...
第一条命令分别检查三个交易所的 BTC、ETH、SOL 永续合约行情，第二条验证自动模式。检查只读取公开行情接口，将图标写入临时目录，不使用现有用户缓存。

单独验证 WebSocket 推送（不混入 HTTP 兜底结果）：

```powershell
uv run --locked python tools/check_streams.py
uv run --locked python tools/check_streams.py --source auto
uv run --locked python tools/check_streams.py --direct
```

默认使用 SOCKS5 `127.0.0.1:7897`，`--direct` 使用直连，`--timeout` 设置等待秒数（默认 20）。任何订阅超时均以非零退出码报告。
